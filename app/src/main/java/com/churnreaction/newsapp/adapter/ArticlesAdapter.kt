package com.churnreaction.newsapp.adapter
...
import com.churnreaction.newsapp.databinding.ItemArticlePreviewBinding
    ...
    inner class ArticleViewHolder(private val binding: ItemArticlePreviewBinding): RecyclerView.ViewHolder(binding.root) {
        init {
            binding.apply {
                ...
                root.shareImage.setOnClickListener{
                    val intent = Intent(Intent.ACTION_SEND)
                    intent.type = "text/plain"

                    intent.putExtra(Intent.EXTRA_TEXT, getItem(position).content)

                    root.context.startActivity(Intent.createChooser(intent, "Share with"))
